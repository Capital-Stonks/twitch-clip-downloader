...
const ensureEnvironmentKeyIsLoaded = (key) => {
  if (!process.env[key]) {
    printErrorsAndExit(
      `Environment variable ${chalk.underline.blue(key)} not set!`,
...
const createIfEnvNotSet = async () => {
  ...
  if (!envExists) {
    console.log(
      `Looks like you haven't set your ${chalk.cyan(".env")} file yet!`
    );
    console.log(
      `We'll now generate this file, so have your Twitch ${chalk.green(
        "CLIENT_ID"
      )} and ${chalk.green("CLIENT_SECRET")} ready!\n`
    );
    console.log("If you don't have these at hand, here's a guide:");
    console.log(
      `Register an application on ${chalk.magenta(
        "Twitch"
      )} Console: ${chalk.underline.blue("https://dev.twitch.tv/console/apps")}`
    );
    console.log(
      `Click ${chalk.cyan("Manage")} and copy the ${chalk.green(
        "CLIENT_ID"
      )} and generate a ${chalk.green("CLIENT_SECRET")}.\n\n`
    );
